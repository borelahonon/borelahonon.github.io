...
document.addEventListener('DOMContentLoaded', () => {
    const animateElements = document.querySelectorAll('.paper, .contact-item, .education-item, .reference-item, .about-text, .cv-download');
...
// Add staggered animation for papers
document.addEventListener('DOMContentLoaded', () => {
...
document.getElementById('cv-download-btn').addEventListener('click', (e) => {
    // Le téléchargement se fait automatiquement grâce à l'attribut download
    // Ajout d'un effet visuel pour confirmer le téléchargement
    const btn = e.target.closest('.btn');
    btn.style.transform = 'scale(0.95)';

    // Ajout d'une notification visuelle
...
function showDownloadNotification() {
    ...
    notification.style.cssText = `
        ...
        top: 20px;
        right: 20px;
        background: linear-gradient(135deg, #0e4e78, #2c5aa0);
        color: white;
        padding: 1rem 1.5rem;
        border-radius: 8px;
